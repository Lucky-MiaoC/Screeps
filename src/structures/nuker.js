...
StructureNuker.prototype.testLaunchNuke = function (pos) {
    ...
    console.log(`测试发射距离...${Game.map.getRoomLinearDistance(this.room.name, pos.roomName) <= 10 ? '通过！' : (flag = 1, '未通过！')}`);
    console.log(`测试目标房间开发状态...${Game.map.getRoomStatus(pos.roomName).status == 'normal' ? '通过！' : (flag = 1, '未通过！')}`);
