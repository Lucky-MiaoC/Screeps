 ...
 * 注意：本人目前只使用到了Source旁边的sourceLink（多个）和中央集群的centerLink（单个）
 * 注意：本人在index.js中只设计了sourceLink、centerLink、upgradeLink三种子类（都返回数组）
 * 注意：如果需要其他种类的Link（例如房间入口专供外矿creep使用的link），你可能需要修改index.js的建筑索引
 */
