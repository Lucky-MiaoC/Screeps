export const roleFiller = {
    run: function (creep) {
        ...
        if (!target
            ...
            || (creep.memory.fillTask == 'energy' &&
                !(target instanceof StructurePowerSpawn) && target.store.getFreeCapacity(RESOURCE_ENERGY) == 0)) {
            target = null;
            creep.memory.targetId = null;
            creep.memory.fillTask = null;
        }
        ...
        if (creep.store[RESOURCE_ENERGY] > 0) {
            ...
            target = target
                ...
                || creep.pos.findClosestByRange(_.filter(creep.room.lab, (i) => {
                    return i.store.getFreeCapacity(RESOURCE_ENERGY) > 1000;
                }))
        ...
        // 如果creep手上啥都没拿
            ...
            if (target) {
                ...
                // 没有energy类型的工作，观测是否有power、ghodium类型工作
        ...
        function fillWork(resourceType) {
            if (!creep.memory.ready) {
                ...
                // Power和G有就拿去送，毕竟稀有可能经常拿不满身体（但都是能拿多少拿多少，反正多的会送回来）
                    ...
                    source = source
                        || ((creep.room.storage && creep.room.storage.store[resourceType] > 0) ?
                            creep.room.storage : null)
                        || ((creep.room.terminal && creep.room.terminal.store[resourceType] > 0) ?
...
export const roleFiller = {
    run: function (creep) {
        ...
        target = target
            ...
            || ((creep.room.nuker && creep.room.nuker.store.getFreeCapacity(RESOURCE_ENERGY) > 0) ? creep.room.nuker : null)
            || ((creep.room.powerSpawn && creep.room.powerSpawn.store.getFreeCapacity(RESOURCE_ENERGY) > 500) ? creep.room.powerSpawn : null)
            || ((creep.memory.ready && creep.room.storage && creep.room.storage.store.getFreeCapacity(RESOURCE_ENERGY) > 0) ? creep.room.storage : null)
            || ((creep.memory.ready && creep.room.terminal && creep.room.terminal.store.getFreeCapacity(RESOURCE_ENERGY) > 0) ? creep.room.terminal : null)
            || (creep.memory.ready && creep.pos.findClosestByRange(_.filter(creep.room.sourceContainer, (i) => {
                return i.store.getFreeCapacity(RESOURCE_ENERGY) > 0;
            })));
